...
import { Task as ApiTask } from './apiTypes';

// Gantt 차트용 Task 인터페이스 (gantt-task-react 라이브러리 호환)
...
// 백엔드 API Task를 Gantt Task로 변환하는 헬퍼 함수
export function convertToGanttTask(task: ApiTask, projectId?: string): GanttTask {
  return {
    id: task.task_id,
    name: task.name,
...
// 백엔드 API Task 타입을 사용 (apiTypes.ts에서 import)
...
export const mockHierarchicalWBSTasks: ApiTask[] = [
  {
    task_id: '1',
    parent_id: null,
    name: '1.0 프로젝트 기획',
...
export const mockProjectTasks: Record<string, ApiTask[]> = {
