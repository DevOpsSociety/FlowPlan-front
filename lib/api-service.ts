...
class ApiService {
  ...
  async createProject(project: Partial<CollaborativeProject>): Promise<ApiResponse<CollaborativeProject>> {
    return this.request<CollaborativeProject>("/projects", {
      method: "POST",
  ...
  async updateProject(id: string, updates: Partial<CollaborativeProject>): Promise<ApiResponse<CollaborativeProject>> {
    return this.request<CollaborativeProject>(`/projects/${id}`, {
      method: "PUT",
  ...
  async createTask(projectId: string, task: Partial<CollaborativeTask>): Promise<ApiResponse<CollaborativeTask>> {
    return this.request<CollaborativeTask>(`/projects/${projectId}/tasks`, {
      method: "POST",
  ...
  async getComments(taskId: string): Promise<ApiResponse<Comment[]>> {
    ...
    const mockCommentsData = await import("./collaboration-mock-data")
  ...
  async addComment(taskId: string, content: string, parentId?: string): Promise<ApiResponse<Comment>> {
  ...
  // 파일 첨부 관련
  async uploadFile(file: File, taskId?: string, commentId?: string): Promise<ApiResponse<FileAttachment>> {
  ...
  async getTeamMembers(projectId: string): Promise<TeamMember[]> {
    const mockTeamMembersData = await import("./collaboration-mock-data")
  ...
  async updateTeamMemberRole(projectId: string, memberId: string, newRole: UserRole): Promise<void> {
  ...
  async getNotifications(): Promise<ApiResponse<Notification[]>> {
    ...
    const mockNotificationsData = await import("./collaboration-mock-data")
  ...
  async getActivityLog(projectId: string): Promise<ApiResponse<ActivityLog[]>> {
    ...
    const mockActivityData = await import("./collaboration-mock-data")
    const projectActivities = mockActivityData.mockActivityLogs.filter((log) => log.projectId === projectId)
